...
export default async function EmailPage() {
  ...
  const [{ data: clients }, { data: leads }, { data: orgUser }] = await Promise.all([
    ...
    supabase.from('users').select('full_name, email, organization_id, email_signature, email_signature_image').single(),
  ])
  ...
  return (
    <EmailClient
      ...
      senderSignatureImage={(orgUser as { email_signature_image?: string | null } | null)?.email_signature_image ?? ''}
    />
  )
}
